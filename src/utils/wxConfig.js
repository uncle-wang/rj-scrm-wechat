...
const getToken = async () => {
  ...
  const res = await request({
    ...
    params: {
      corpid: 'ww851e9d2540b5ed3c',
      corpsecret: '5mqhNGn0Fa4gtDXticaiKMhT04lU0ez9j8vfrHYtuK8'
    },
  });
  if (res.errcode === 0) {
...
const wxConfig = () => new Promise((resolve, reject) => {
  getTicket().then(ticket => {
    const options = {
      ...
      appId: 'ww851e9d2540b5ed3c', // 必填，企业微信的corpID
...
const agentConfig = () => new Promise((resolve, reject) => {
  wxConfig().then(getAppTicket).then(ticket => {
    const options = {
      corpid: 'ww851e9d2540b5ed3c', // 必填，企业微信的corpid，必须与当前登录的企业一致
